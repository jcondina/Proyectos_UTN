    ...
    ui(new Ui::Dialog)
{
    ...
    //connect(&MainWindow,SIGNAL(SendData_micro(QString),this,SLOT(ReceivedData_micro(QString)) );
    //connect(Dialog::onReadyRead , SIGNAL(SendData_dialog(QString)), nuevo_usuario, SLOT(ReceivedData_dialog(QString)));
    //! [0]
...
void Dialog::onReadyRead()
{
    if (port->bytesAvailable()) {
        ...
        rfid_dialog = QString::fromLatin1(port->readAll());
        qDebug()<<rfid_dialog;
        if(rfid_dialog.size()==14)
        {
            emit SendData_dialog(rfid_dialog);
            emit SendData_iotarjeta(rfid_dialog);
        }

        if(rfid_dialog.size()<4)
            emit SendData_io(rfid_dialog);

        if(rfid_dialog.size()==14)
        {
                char item[14];
                strcpy(item,rfid_dialog.toUtf8());
                int i;
                ...
                db.setDatabaseName("info2");
                db.setUserName("root");
                db.setPassword("argentina1234");
                bool ok = db.open();
